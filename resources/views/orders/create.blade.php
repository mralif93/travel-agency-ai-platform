                            ...
                            <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                                <div>
                                    ...
                                    <div class="text-3xl font-mono font-bold text-gray-900 dark:text-white"
                                        id="display_distance">0.00 km</div>
        ...
        function calculateDistanceAndPrice() {
            ...
            fetch(url).then(res => res.json()).then(data => {
                if (data.routes && data.routes.length > 0) {
                    ...
                    // Update text
                    document.getElementById('display_distance').textContent = `${dist} km`;
                    document.getElementById('distance_km').value = dist;
                    let price = dist <= 60 ? dist * 2.50 : (60 * 2.50) + ((dist - 60) * 1.20);
                    document.getElementById('display_price').textContent = `RM ${price.toFixed(2)}`;
                    ...
                    const blueIcon = L.divIcon({
                        ...
                        html: `<div class="w-4 h-4 rounded-full bg-blue-600 ring-4 ring-white shadow-lg"></div>`
                    });
                    const redIcon = L.divIcon({
                        className: 'bg-transparent',
                        html: `<div class="w-4 h-4 rounded-full bg-red-600 ring-4 ring-white shadow-lg"></div>`
                    });
